    ...
    DataTable detalle, DateTime dCmp_Fecha_Vencimiento)
        {
            using (OdbcConnection conn = conexion.conexion())
            {
                ...
                try
                {
                    ...
                    using (OdbcCommand cmd = new OdbcCommand(SQL_UPDATE_VENTA, conn, trans))
                    {
                    ...
                    // SOLO MODIFICAR DETALLE SI NO ES CONVERSIÓN A VENTA
                    if (!convertirAVenta)
                    {
                        // OBTENER DETALLE ORIGINAL
                        DataTable detalleOriginal = ObtenerDetalleOriginal(iPk_Id_Venta);

                        // DEVOLVER STOCK
                        foreach (DataRow row in detalleOriginal.Rows)
                        {
                            int idProducto = Convert.ToInt32(row["Fk_Id_Inventario"]);
                            float cantidad = Convert.ToSingle(row["Cmp_Cantidad_Producto"]);

                            using (OdbcCommand cmdStock = new OdbcCommand(
                        ...
                        using (OdbcCommand cmdDelete = new OdbcCommand(
                            @"DELETE FROM tbl_detalle_ventas
                      WHERE Fk_Id_Ventas = ?",
                            conn, trans))
                        {
                            cmdDelete.Parameters.AddWithValue("?", iPk_Id_Venta);
                        ...
                        foreach (DataRow row in detalle.Rows)
                        {
                            ...
                            {
                                cmdDetalle.Parameters.AddWithValue("?", iPk_Id_Venta);
                    ...
                    if (convertirAVenta)
                    {
                        bool yaTieneCXC = ExisteCuentaPorCobrar(iPk_Id_Venta);
                        ...
                        if (!yaTieneCXC)
                        {
                            ...
                            bool movimiento = inventario.fun_GuardarMovimiento(
                                3,
                                dCmp_Fecha_Venta,
                                "Venta",
                                detalleInventario
                          ...
                          WHERE Pk_Id_Cliente = ?",
                                conn, trans))
                            {
                                cmdCliente.Parameters.AddWithValue("?", fCmp_Saldo_Total);
                                cmdCliente.Parameters.AddWithValue("?", iFk_Id_Cliente);
                                cmdCliente.ExecuteNonQuery();
                            }

                            // CREAR CUENTA POR COBRAR
                            using (OdbcCommand cmdCXC = new OdbcCommand(SQL_INSERT_CUENTA_COBRAR, conn, trans))
                            {
                                cmdCXC.Parameters.AddWithValue("?", iPk_Id_Venta);
                                cmdCXC.Parameters.AddWithValue("?", iFk_Id_Cliente);
                                cmdCXC.Parameters.AddWithValue("?", dCmp_Fecha_Venta);
                                cmdCXC.Parameters.AddWithValue("?", dCmp_Fecha_Vencimiento);
                                cmdCXC.Parameters.AddWithValue("?", fCmp_Saldo_Total);
                                cmdCXC.Parameters.AddWithValue("?", "Activo");
                ...
                {
                    trans.Rollback();
                    throw new Exception("Error actualizar venta: " + ex.Message);
                }
                finally
                {
                    conexion.desconexion(conn);
